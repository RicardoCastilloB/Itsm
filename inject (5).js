/**
 * inject.js  v2
 * Lee CSV (TSV) e inserta datos en: departments, locations, employees, equipment, assignments
 *
...
const C = {
  ...
  branch_office_id:31, state:32, desc_ceo:33, desc_ceo_1:34,
  desc_ceo_2:35, desc_ceo_3:36, desc_ceo_4:37, desc_ceo_5:38,
  desc_ceo_6:39, desc_ceo_7:40,
};

const v     = (row, k) => { const x = row[C[k]]; return (x !== undefined && x.trim() !== "") ? x.trim() : null; };
const toInt = s => { if (!s) return null; const n = parseInt(s.replace(/\D/g,""),10); return isNaN(n)?null:n; };

const mapStatus = s => {
  if (!s) return "Activo";
  if (s.toLowerCase().includes("desasign")) return "Inactivo";
  return "Activo";
};
const mapAcq = s => {
  if (!s) return null;
  ...
  if (l.includes("presta"))  return "Prestado";
  return s;
};
const mapEqType = s => {
  if (!s) return null;
  ...
  if (l.includes("celular")||l.includes("movil"))         return "Celular";
  return s;
};

// ─── CACHES (evita SELECT repetidos) ──────────────────────────────────────────
const cacheDept = new Map();
const cacheLoc  = new Map();
const cacheEmp  = new Map();
const cacheEqp  = new Map();
...
async function getDept(conn, row) {
  ...
  if (rows.length) { cacheDept.set(name, rows[0].id); return rows[0].id; }
  ...
  const [r] = await conn.execute(
    `INSERT INTO departments
     ...
     VALUES (?,?,?,1,?,?,?,?,?,?,?,?)`,
    [name, v(row,"division"), v(row,"subactivity"),
     v(row,"desc_ceo"),  v(row,"desc_ceo_1"), v(row,"desc_ceo_2"),
     v(row,"desc_ceo_3"), v(row,"desc_ceo_4"), v(row,"desc_ceo_5"),
     v(row,"desc_ceo_6"), v(row,"desc_ceo_7")]
  );
...
async function getLoc(conn, row) {
  ...
  if (rows.length) { cacheLoc.set(branch, rows[0].id); return rows[0].id; }
  ...
  cacheLoc.set(branch, r.insertId);
  return r.insertId;
}
...
async function getEmp(conn, row, deptId) {
  ...
  if (rows.length) {
    await conn.execute(
      `UPDATE employees SET
       ...
       WHERE cip=?`,
      [v(row,"national_id"), v(row,"document_type")||"DNI", v(row,"full_name"),
       v(row,"email"), v(row,"network_account"), v(row,"position_name"),
       v(row,"category"), v(row,"employee_group"), v(row,"legal_entity"),
       v(row,"supervisor_name"), v(row,"branch_office_id"), v(row,"state"),
       deptId, cip]
    );
  ...
  const [r] = await conn.execute(
    `INSERT INTO employees
     ...
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)`,
    [cip, v(row,"national_id"), v(row,"document_type")||"DNI",
     v(row,"full_name"), v(row,"email"), v(row,"network_account"),
     v(row,"position_name"), v(row,"category"), v(row,"employee_group"),
     v(row,"legal_entity"), v(row,"supervisor_name"),
     v(row,"branch_office_id"), v(row,"state"), deptId]
  );
  cacheEmp.set(cip, r.insertId);
  return r.insertId;
}
...
async function getEqp(conn, row) {
  ...
  if (rows.length) {
    await conn.execute(
      `UPDATE equipment SET
       ...
       WHERE device_code=?`,
      [v(row,"model"), v(row,"serial_number"), v(row,"processor"),
       v(row,"operating_system"), v(row,"disk_capacity"), v(row,"ram_memory"),
       mapEqType(v(row,"equipment_type")), v(row,"brand"),
       mapAcq(v(row,"acquisition_type")), toInt(v(row,"obsolescence_years")),
       v(row,"domain"), v(row,"it_level_1"), v(row,"it_level_2"), code]
    );
  ...
  const [r] = await conn.execute(
    `INSERT INTO equipment
     ...
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,'Disponible')`,
    [code, v(row,"model"), v(row,"serial_number"), v(row,"processor"),
     v(row,"operating_system"), v(row,"disk_capacity"), v(row,"ram_memory"),
     mapEqType(v(row,"equipment_type")), v(row,"brand"),
     mapAcq(v(row,"acquisition_type")), toInt(v(row,"obsolescence_years")),
     v(row,"domain"), v(row,"it_level_1"), v(row,"it_level_2")]
  );
  cacheEqp.set(code, r.insertId);
  return r.insertId;
}
...
async function doAssignment(conn, row, empId, eqpId, deptId, locId) {
  ...
  if (rows.length) {
    await conn.execute(
      "UPDATE assignments SET status=?,relation_type=?,department_id=?,location_id=? WHERE id=?",
      [mapStatus(v(row,"status")), v(row,"relation_type"), deptId, locId, rows[0].id]
    );
    return;
  }
  ...
  await conn.execute(
    `INSERT INTO assignments
     ...
     VALUES (?,?,?,?,?,?,?,CURDATE())`,
    [empId, eqpId, deptId, locId, period,
     v(row,"relation_type"), mapStatus(v(row,"status"))]
  );
}
...
async function main() {
  ...
  if (!csvPath) { console.error("❌  Uso: node inject.js <archivo.csv>"); process.exit(1); }
  ...
  // Conexión simple — sin pool, sin transacciones por fila
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    try {
      ...
      if (ok % 100 === 0) {
        const [[{cnt}]] = await conn.execute("SELECT COUNT(*) AS cnt FROM employees");
        console.log(`  ✅  ${ok} procesadas | employees en BD ahora: ${cnt}`);
      }
    } catch (err) {
      errors++;

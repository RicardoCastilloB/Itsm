...
async function processRow(conn, row) {
  ...
  // Actualizar department_id por nombre solo si existe
  if (v(row['department_name'])) {
    await conn.execute(`
      UPDATE employees e
  ...
  const deptNameForAssign = v(row['department_name']);
  const [[deptRow]] = deptNameForAssign
    ? await conn.execute(`SELECT id FROM departments WHERE department_name = ? LIMIT 1`, [deptNameForAssign])
    : [[null]];
  const [[locRow]] = branchId
